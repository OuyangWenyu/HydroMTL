"""
Plot for Deep Learning training or test results
...
def plot_df_test_with_probabilistic_confidence_interval(
    df_test: pd.DataFrame, forecast_start_index: int, params: Dict, real_data=True
) -> go.Figure:
    fig = go.Figure()
    target_col = params["data_params"]["target_col"][0]
    fig.add_trace(go.Scatter(x=df_test.index, y=df_test["preds"], name="preds"))
    if real_data:
        fig.add_trace(
    ...
    fig.add_trace(
        go.Scatter(
            x=df_test.index,
            y=df_test["preds"] + 2 * df_test["std_dev"],
            name="upper bound",
        )
    )
    fig.add_trace(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=[
                min(
                    (df_test["preds"] - df_test["std_dev"]).min(),
                    df_test[target_col].min(),
                ),
                max(
                    (df_test["preds"] + df_test["std_dev"]).max(),
                    df_test[target_col].max(),
                ),
            ],
            name="pred_start",
        )
