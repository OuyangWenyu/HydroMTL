...
LastEditTime: 2023-01-04 10:14:42
LastEditors: Wenyu Ouyang
Description: Some util functions for scripts in app/streamflow
FilePath: \HydroSPB\hydroSPB\app\streamflow_utils.py
...
import definitions
from hydroSPB.utils.hydro_utils import unserialize_json, random_choice_no_return
from hydroSPB.data.data_dict import data_sources_dict
from hydroSPB.hydroDL.trainer import (
    load_result,
    save_result,
    stat_result,
    train_and_evaluate,
)
from hydroSPB.utils.hydro_stat import remove_abnormal_data, stat_error, ecdf
from hydroSPB.data.config import default_config_file, update_cfg, cmd
from hydroSPB.data.source.data_gages import Gages
from hydroSPB.data.source_pro.select_gages_ids import choose_sites_in_ecoregion
from hydroSPB.visual.plot_stat import plot_ecdfs_matplot, plot_ts, plot_rainfall_runoff
from hydroSPB.app.app_constant import VAR_C_CHOSEN_FROM_GAGES_II
from hydroSPB.utils import hydro_constant, hydro_utils
...
def show_dpl_result(exp, flow_unit, exp_dir="camels", source_data_dir=None):
    """
    statistics of result for exp
     ...
     flow_unit
         trans the unit of streamflow to m3/s
     exp_dir
         where exps are saved, default is camels
     source_data_dir
         sometimes we will use files copied from other device, so the dir is not correct for this device
         we need manually set the directory

    """
    cfg_dir_flow = os.path.join(
        definitions.ROOT_DIR, "hydroSPB", "example", exp_dir, exp
    )
    cfg_flow = get_json_file(cfg_dir_flow)
    if cfg_flow["data_params"]["test_path"] != cfg_dir_flow:
        # sometimes we will use files copied from other device, so the dir is not correct for this device
        cfg_flow["data_params"]["test_path"] = cfg_dir_flow
        cfg_flow["data_params"]["data_path"] = source_data_dir
    print("load results")

    data_params = cfg_flow["data_params"]
    ...
    inds_df, pred, obs = stat_result(
        ...
        cfg_flow["evaluate_params"]["test_epoch"],
        fill_nan=cfg_flow["evaluate_params"]["fill_nan"],
        return_value=True,
        unit=flow_unit,
        basin_area=data_source.read_basin_area(sites),
    )
    inds_df["gage_id"] = sites
    return inds_df, pred, obs, cfg_flow
...
def get_latest_dirs_for_sceua_xaj(exp, comment, cv_fold=2):
    ...
    """
    all_dirs = os.listdir(
        os.path.join(definitions.ROOT_DIR, "hydroSPB", "example", exp)
    )
    dirs = []
    for i in range(cv_fold):
        chosen_dirs = [
...
def read_sceua_xaj_metric(basin_id, comment):
    """read SCEUA-XAJ from hydro-model-xaj project
    ...
    basin_id : _type_
        the id of a basin
    comment : _type_
        the comment as an end of an exp directory's name
    """
    ...
    print("The metrics of training results of basin " + basin_id + " are:")
    print(basin_id_train_metric)
    print("The metrics of testing results of basin " + basin_id + " are:")
    print(basin_id_test_metric)
    return basin_id_train_metric, basin_id_test_metric


def get_p_pet_q_df_from_an_sceua_xaj_exp(exp):
    data_dir = os.path.join(definitions.ROOT_DIR, "hydroSPB", "example", "gages", exp)
    ...
    for i in range(p_pet_q.shape[1]):
        pd.DataFrame(
            ...
            os.path.join(data_dir, f'basin_{data_info["basin"][i]}.csv'),
            index_label="time",
            header=data_info["variable"],
        )


def read_sceua_xaj_streamflow(basin_id, best_plan, kfold, warmup=365, basin_area=None):
    qsim_train_result_lst = []
    qsim_test_result_lst = []
    qobs_train_result_lst = []
    qobs_test_result_lst = []
    comment_dir = get_latest_dirs_for_sceua_xaj(
        "gages" + os.sep + "exp" + basin_id, best_plan, kfold
    )
    for i in range(kfold):
        ...
        qsim_train_result = pd.read_csv(train_result_file, index_col=0)
        qsim_test_result = pd.read_csv(test_result_file, index_col=0)
        qsim_train_result_lst.append(qsim_train_result)
        qsim_test_result_lst.append(qsim_test_result)
        train_data_info_file = os.path.join(
            data_dir, f"data_info_fold{str(i)}_train.json"
        )
        train_data_file = os.path.join(
            data_dir, f"basins_lump_p_pe_q_fold{str(i)}_train.npy"
        )
        test_data_info_file = os.path.join(
            data_dir, f"data_info_fold{str(i)}_test.json"
        )
        test_data_file = os.path.join(
            data_dir, f"basins_lump_p_pe_q_fold{str(i)}_test.npy"
        )
        ...
        # NOTE: index of 0 means only one basin
        qobs_train = data_train[
            warmup:, 0, data_info_train["variable"].index("streamflow(mm/day)")
        ]
        qobs_test = data_test[
            warmup:, 0, data_info_test["variable"].index("streamflow(mm/day)")
        ]
        qobs_train_ = hydro_constant.convert_unit(
            qobs_train,
            "mm/day",
            hydro_constant.unit["streamflow"],
            basin_area=basin_area,
        )
        qobs_test_ = hydro_constant.convert_unit(
            qobs_test,
            "mm/day",
            hydro_constant.unit["streamflow"],
            basin_area=basin_area,
        )
        qobs_train_result = pd.DataFrame(
            qobs_train_,
            index=pd.to_datetime(data_info_train["time"])
            .values[warmup:]
            .astype("datetime64[D]"),
            columns=[basin_id],
        )
        qobs_test_result = pd.DataFrame(
            qobs_test_,
            index=pd.to_datetime(data_info_test["time"])
            .values[warmup:]
            .astype("datetime64[D]"),
            columns=[basin_id],
        )
        qobs_train_result_lst.append(qobs_train_result)
        qobs_test_result_lst.append(qobs_test_result)
    return (
        qsim_train_result_lst,
        qsim_test_result_lst,
        qobs_train_result_lst,
...
def read_sceua_xaj_et_metric(basin_id, comment, cv_fold=2, et_type=ET_MODIS_NAME):
    inds_df_trains_lst = []
    inds_df_valids_lst = []
    comment_dir = get_latest_dirs_for_sceua_xaj(
        "gages/exp" + basin_id, comment, cv_fold
    )
    for i in range(cv_fold):
        (
            pred_train_,
            ...
            obs_train_,
            obs_valid_,
        ) = read_sceua_xaj_et_for_1basin1fold(basin_id, comment_dir[i], et_type)
...
def read_sceua_xaj_et(basin_id, best_plan, kfold, et_type=ET_MODIS_NAME):
    """Read SCEUA XAJ ET simulation results.
    ...
    -------
    list, list
        train and test ET simulation results
    """
    et_sim_train_lst = []
    et_sim_test_lst = []
    et_obs_train_lst = []
    et_obs_test_lst = []
    comment_dir = get_latest_dirs_for_sceua_xaj(
        "gages/exp" + basin_id, best_plan, kfold
    )
    for i in range(kfold):
        (
            et_sim_train,
        ...
        et_sim_train_lst.append(et_sim_train)
        et_sim_test_lst.append(et_sim_test)
        et_obs_train_lst.append(et_obs_train_result)
        et_obs_test_lst.append(et_obs_test_result)
    return et_sim_train_lst, et_sim_test_lst, et_obs_train_lst, et_obs_test_lst


def read_sceua_xaj_et_for_1basin1fold(basin_id, comment_dir, et_type=ET_MODIS_NAME):
    data_dir = os.path.join(
        definitions.ROOT_DIR,
        "hydroSPB",
        "example",
    ...
    train_result_file = os.path.join(
        data_dir,
        comment_dir,
        "basin_etsim_train.csv",
    )
    test_result_file = os.path.join(
        data_dir,
        comment_dir,
        "basin_etsim_test.csv",
    )
    et_sim_train = pd.read_csv(train_result_file, index_col=0)
    et_sim_test = pd.read_csv(test_result_file, index_col=0)

    t_train_lst = pd.date_range(
        start=np.datetime64(et_sim_train.index[0]),
        end=np.datetime64(et_sim_train.index[-1]) + np.timedelta64(1, "D"),
        freq="D",
    )
    t_range_train = hydro_utils.t_days_lst2range(t_train_lst)
    t_test_lst = pd.date_range(
        np.datetime64(et_sim_test.index[0]),
        np.datetime64(et_sim_test.index[-1]) + np.timedelta64(1, "D"),
        freq="D",
    )
    t_range_test = hydro_utils.t_days_lst2range(t_test_lst)
    et_obs_train = read_et_obs_for_1basin(
        basin_id=basin_id, period=t_range_train, et_type=et_type
    )
    et_obs_test = read_et_obs_for_1basin(
        basin_id=basin_id, period=t_range_test, et_type=et_type
    )
    et_obs_train_result = pd.DataFrame(
        et_obs_train.flatten(),
        index=et_sim_train.index,
        columns=[basin_id],
    )
    et_obs_test_result = pd.DataFrame(
        et_obs_test.flatten(),
        index=et_sim_test.index,
        columns=[basin_id],
    )
    return et_sim_train, et_sim_test, et_obs_train_result, et_obs_test_result


def read_et_obs_for_1basin(basin_id, period, et_type=ET_MODIS_NAME):
    """Read ET obs for 1 basin in CAMELS_CC.
    ...
    pandas.DataFrame
        ET obs
    period
        the list of a period
    """
    camels_cc = Camels(
        os.path.join(definitions.DATASET_DIR, "camels", "camels_cc"), region="CC"
    )
    if et_type == ET_MODIS_NAME:
        et_obs = camels_cc.read_target_cols([basin_id], period, [ET_MODIS_NAME]) * 0.1
    elif et_type == ET_ERA5LAND_NAME:
        et_obs_ = camels_cc.read_relevant_cols([basin_id], period, [ET_ERA5LAND_NAME])
        et_obs = remove_abnormal_data(et_obs_, q1=0)
    else:
        raise ValueError("ET type not supported!")
    return et_obs
...
def plot_xaj_lstm_tl_dpl_rainfall_runoff(
    ...
    t_periods = hydro_utils.t_range_days(time_range)
